...
const Studio = (props) => {
    ...
    useEffect(() => {
        if (props.transcribeFile) {
            ee.emit('newtrack', {
                file: props.transcribeFile,
                name: props.transcribeFile.name,
                id: uuidv4(),
            });
            ee.emit('newtrack', {
                file: props.dubbingFile,
                name: props.dubbingFile.name,
                id: uuidv4(),
            });
        }

    }, [props.transcribeFile])
    useEffect(() => {
        if(props.subTitle && props.subTitle.length > 0)
        ee.emit('addAnnotation', props.subTitle);
        setShowAnnotations(true);
        setEnableAnnotations(false);
    }, [props.subTitle])
    ...
    return (
        <div className='h-[300px] overflow-y-scroll'>
            ...
            <div class="absolute left-0 top-0 p-3 w-full bg-[#21222D] border-t-2" style={{ zIndex: 1000 }}>
                <EditorButtons
            ...
            />
            <CustomAudioBar bottom={!allbuttons ? 0 : -100} ee={ee} setCurrentTime={props.setCurrentTime}/>
        </div>
